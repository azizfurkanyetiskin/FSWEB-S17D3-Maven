...
public class KoalaController {
    ...
    @GetMapping
    public List<Koala> findAll() { return new ArrayList<>(koalas.values()); }

    @GetMapping("/{id}")
    public Koala findById(@PathVariable int id) {
        if (!koalas.containsKey(id)) throw new ZooException("Koala with id " + id + " not found", HttpStatus.NOT_FOUND);
